...
function drawPrecip() {
  ...
  for(var p = 0; p < precip.length; p++) {
    if (p == index && mouseX>minX && mouseX<maxX && mouseY<height*3/4 && mouseY>height/4) {
      // Gold color if mouse is over it
      stroke(220,190,50);
    } else {
      stroke(90,120,230);
    }
...
function labels() {
  ...
  if (mouseX > minX && mouseX < maxX) {
    ...
    if (mouseY < height/2 && mouseY > height/4) { // Display temperature
      text(temps[index], index*spacing+minX-3, height/2 - (tempFactor)*temps[index] - 20);
    } else if (mouseY >= height/2 && mouseY < height*3/4) { // Display precipitation
      text(precip[index], index*spacing+minX-5, height/2 + (precipFactor)*precip[index] + 40);
    }
...
function titles() {
  ...
  textSize(25);
  fill(220);
  if (mouseX > minX && mouseX < maxX && mouseY < height*3/4 && mouseY > height/4) {
