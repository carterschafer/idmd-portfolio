...
function getNumDays(monthIndex) {
  var count = 0; // count how many moths have been added
  var dayTotal = 0; // days up to the given month
  
  // for loop to iterate from 0 up to the monthIndex
  for (i = 0; i < monthIndex; i++) {
    dayTotal = dayTotal + getDaysInMonth(i); // adding number of days in the current month to dayTotal
  }
  return dayTotal; // return dayTotal when we have added everything necessary
}
...
function getDaysInMonth(month) {
  if  (month == 1) { //month is february, return 29
    return 29;
  } else if (month == 3 || month == 5 || month == 8 || month == 10) { //if any of these months, return 30
    return 30;
  } else { //otherwise return 31
    return 31;
  }
